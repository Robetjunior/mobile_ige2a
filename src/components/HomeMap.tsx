...
const DEFAULT_CENTER = { lat: -23.5614, lng: -46.6559 };
...
export const HomeMap: React.FC<HomeMapProps> = ({
  ...
  const markers = useMemo(() => {
    ...
    // Fallback de mock
    const mock = [
      { id: 'DRBAKANA-TEST-01', name: 'DRBAKANA-TEST-01', lat: -23.5614, lng: -46.6559, status: 'online' },
      { id: 'DRBAKANA-TEST-02', name: 'DRBAKANA-TEST-02', lat: -23.5650, lng: -46.6535, status: 'busy' },
      { id: 'DRBAKANA-TEST-03', name: 'DRBAKANA-TEST-03', lat: -23.5591, lng: -46.6601, status: 'offline' },
    ];
    return mock.map((m) => ({
      id: m.id,
      name: m.name,
      ...
      station: { id: m.id, name: m.name, address: '', status: m.status as any } as Station,
    }));
  }, [stations]);
  ...
  if (loadError) {
    return (
      <View style={{ flex: 1, backgroundColor: '#F3F4F6', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {!apiKey && (
          <Text style={{ color: '#6B7280', marginTop: 6 }}>Defina NEXT_PUBLIC_GOOGLE_MAPS_API_KEY</Text>
        )}
      </View>
    );
  }
